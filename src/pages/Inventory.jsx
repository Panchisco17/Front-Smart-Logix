...
function InventoryPage() {
    ...
    return (
        <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
            <h2 className="text-xl font-bold mb-6 text-gray-800">Gestión de Inventario</h2>

            {(userRole === "ROLE_ADMIN" || userRole === "ROLE_WAREHOUSE_MANAGER") && (
                <div className="bg-gray-50 p-4 rounded-lg border mb-6">
                    <h3 className="font-semibold text-gray-700 mb-4">Agregar Producto</h3>
                    <form onSubmit={handleCreateItem}>
                        <div className="grid grid-cols-2 md:grid-cols-6 gap-4 items-end">
                            ...
                            <div>
                                <label className="block text-xs font-bold text-gray-700 mb-1">Bodega</label>
                            ...
                            <div>
                                <label className="block text-xs font-bold text-gray-700 mb-1">Reorden</label>
                            ...
                            <div className="md:col-span-6 flex justify-end mt-2">
                                <button type="submit" className="bg-green-600 hover:bg-green-700 text-white px-6 py-2 rounded transition font-bold">
                                    + Agregar
                                </button>
                            </div>
                        </div>
            ...
            <div className="overflow-x-auto">
                <table className="w-full text-sm text-left text-gray-600">
                    ...
                    <tbody className="divide-y divide-gray-100">
                        {inventory.length === 0 ? (
                            <tr><td colSpan="7" className="text-center py-6 text-gray-500">No existen productos registrados.</td></tr>
                        ) : (
                            inventory.map((item) => (
                                <tr key={item.sku} className="bg-white hover:bg-gray-50 transition-colors">
                                    ...
                                    <td className="px-6 py-4"><span className="bg-blue-100 text-blue-800 text-xs font-semibold px-2.5 py-0.5 rounded">{item.warehouseCode}</span></td>
